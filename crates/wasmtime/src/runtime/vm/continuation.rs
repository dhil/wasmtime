...
pub mod optimized {
    ...
    pub fn suspend(instance: &mut Instance, tag_index: u32) -> Result<(), TrapReason> {
        ...
        let suspend = crate::vm::fibre::unix::Suspend::from_top_ptr(stack_ptr);
...
pub mod optimized {
    ...
    impl StackChain {
        /// Indicates if `self` is a `MainStack` variant.
        pub fn is_main_stack(&self) -> bool {
            panic!("attempt to execute continuation::optimized::StackChain::is_main_stack with `typed_continuation_baseline_implementation` toggled!")
        }
        ...
        /// whose usage is unsafe.
        pub unsafe fn into_iter(self) -> ContinuationChainIterator {
            panic!("attempt to execute continuation::optimized::StackChain::into_iter with `typed_continuation_baseline_implementation` toggled!")
        }
    }
    ...
    impl Iterator for ContinuationChainIterator {
        ...
        fn next(&mut self) -> Option<Self::Item> {
            panic!("attempt to execute continuation::optimized::ContinuationChainIterator::next with `typed_continuation_baseline_implementation` toggled!")
        }
    }
    ...
    /// Wraps a `StackChain` in an `UnsafeCell`, in order to store it in a
    /// `StoreOpaque`.
    pub struct StackChainCell(pub std::cell::UnsafeCell<StackChain>);

    impl StackChainCell {
        /// Indicates if the underlying `StackChain` object has value `Absent`.
        pub fn absent() -> Self {
            panic!("attempt to execute continuation::optimized::StackChainCell::absent with `typed_continuation_baseline_implementation` toggled!")
        }
    }
    ...
    #[inline(always)]
    pub fn cont_obj_get_cont_ref(_contobj: *mut VMContObj) -> Result<*mut VMContRef, TrapReason> {
        panic!("attempt to execute continuation::optimized::cont_obj_get_cont_ref with `typed_continuation_baseline_implementation` toggled!")
    }
    ...
    #[inline(always)]
    pub fn new_cont_obj(_contref: *mut VMContRef) -> *mut VMContObj {
        panic!("attempt to execute continuation::optimized::new_cont_obj with `typed_continuation_baseline_implementation` toggled!")
    }
